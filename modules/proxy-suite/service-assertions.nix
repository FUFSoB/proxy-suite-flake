...
let
  ...
  featureAssertions = [
    ...
    (uniqueValues
      singBoxCfg.enable
      outboundTags
      "proxy-suite: outbound tags must be unique")
    (uniqueValues
      singBoxCfg.enable
      subscriptionTags
    ...
    (mkAssertion
      (!singBoxCfg.enable || invalidRoutingTargets == [ ])
      "proxy-suite: routing.rules reference unknown outbound tag(s): ${lib.concatStringsSep ", " invalidRoutingTargets}")
    (requireEnabled globalTun.enable singBoxCfg.enable "proxy-suite: singBox.tun.enable requires singBox.enable = true")
    (requireEnabled globalTproxy.enable singBoxCfg.enable "proxy-suite: singBox.tproxy.enable requires singBox.enable = true")
    (requireEnabled globalTun.autostart globalTun.enable "proxy-suite: singBox.tun.autostart requires singBox.tun.enable = true")
    (requireEnabled globalTproxy.autostart globalTproxy.enable "proxy-suite: singBox.tproxy.autostart requires singBox.tproxy.enable = true")
    (mkAssertion
  ...
  perAppRoutingAssertions = [
    ...
    (requireAvailable
      hasZapretProfiles
      (perAppZapretCfg.enable && cfg.zapret.enable)
  ...
  collisionAssertions = map (
    item:
    notEqualWhen item.condition item.left item.right item.message
  ) [
    {
  ...
  forbiddenValueAssertions = map (
    item:
    forbiddenValues item.condition item.value item.disallowed item.message
  ) [
    {
      condition = perAppZapretCfg.enable;
      value = perAppZapretCfg.filterMark;
      disallowed = [ 536870912 1073741824 ];
      message = "proxy-suite: zapret.perApp.filterMark must not use zapret internal desync mark bits";
    }
    {
      condition = perAppZapretCfg.enable;
      value = globalTproxy.fwmark;
      disallowed = [ 67108864 134217728 ];
